...
export function useMetadataEditor(
  ...
  const addField = useCallback((labelStr: string) => {
    ...
    if (knownTopLevel.includes(propName)) {
      ...
      } else if (propName === 'navDate') {
        onUpdate({ navDate: new Date().toISOString() });
      } else if (propName === 'viewingDirection') {
        onUpdate({ viewingDirection: 'left-to-right' });
      } else if (propName === 'requiredStatement') {
        onUpdate({ requiredStatement: { label: { none: ['Attribution'] }, value: { none: [''] } } });
      } else {
