...
const LinkRow: React.FC<LinkRowProps> = ({ item, index, onEdit, onRemove, fieldMode, disabled }) => {
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${
      ...
      <Icon
        name={icon}
        className={`text-base shrink-0 ${fieldMode ? 'text-slate-400' : 'text-slate-500'}`}
      />
    ...
    <div className={`p-3 rounded-lg border space-y-3 ${
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <label className={`block text-xs font-medium mb-1 ${fieldMode ? 'text-slate-400' : 'text-slate-600'}`}>
            Format
          </label>
          <select
            value={format}
