...
export function createKarwaanSequence(): File[] {
  ...
  for (const filename of files) {
    ...
    if (!testFileExists(relativePath)) {
      console.warn(`Karwaan file not found: ${relativePath}, skipping`);
      continue;
    }
...
export function createMultiAngleBatch(): File[] {
  ...
  for (const filename of files) {
    if (!testFileExists(filename)) {
      console.warn(`Multi-angle file not found: ${filename}, skipping`);
      continue;
    }
...
export function createMixedMediaBatch(): File[] {
  ...
  for (const key of imageKeys) {
    ...
    const filename = TEST_IMAGES[key].filename;
    const type = TEST_IMAGES[key].type;
