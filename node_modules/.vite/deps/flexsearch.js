...
function intersect(a, b, c, d) {
  ...
  for (let j = e - 1; 0 <= j; j--) {
    ...
    for (let a2 = 0; a2 < l; a2++) {
      ...
      if (o) for (let a3, k2, p = 0; p < o; p++) if (k2 = l2[p], f) {
        if (f[k2]) {
          if (!j) {
            if (c) c--;
            else if (h[i++] = k2, i === b) return h;
          }
          (j || d) && (m[k2] = 1), n = true;
        }
  ...
  if (d) for (let a2, e2, g2 = d.length - 1; 0 <= g2; g2--) {
    a2 = d[g2], e2 = a2.length;
    for (let d2, g3 = 0; g3 < e2; g3++) if (d2 = a2[g3], !f[d2]) {
      if (c) c--;
      else if (h[i++] = d2, i === b) return h;
      f[d2] = 1;
    }
  }
  return h;
...
CacheClass.prototype.set = function(a, b) {
  if (!this.cache[a]) {
    ...
    for (let a2 = b2 - 1; 0 < a2; a2--) this.queue[a2] = this.queue[a2 - 1];
    this.queue[0] = a;
  }
  this.cache[a] = b;
...
function apply_preset(a) {
  if (is_string(a)) DEBUG && !preset[a] && console.warn("Preset not found: " + a), a = preset[a];
  else {
    const b = a.preset;
...
function exportIndex(a, b, c, d, e) {
  ...
  switch (e || (e = 0)) {
    case 0:
      if (f = "reg", this.fastupdate) for (let a2 in g = create_object(), this.register) g[a2] = 1;
      ...
      for (let j = 0; j < e; j++) {
        ...
        if (i && k >= this.minlength && (g || !f[i])) {
          ...
          switch (this.tokenize) {
            case "full":
              if (2 < k) {
                for (let b2 = 0; b2 < k; b2++) for (let d3 = k; d3 > b2; d3--) if (d3 - b2 >= this.minlength) {
            ...
            case "reverse":
              if (1 < k) {
                for (let b2 = k - 1; 0 < b2; b2--) if (m = i[b2] + m, m.length >= this.minlength) {
            ...
            case "forward":
              if (1 < k) {
                for (let b2 = 0; b2 < k; b2++) m += i[b2], m.length >= this.minlength && this.push_index(f, m, l, a, c);
                break;
              }
            default:
              if (this.boost && (l = Math.min(0 | l / this.boost(b, i, j), h - 1)), this.push_index(f, i, l, a, c), g && 1 < e && j < e - 1) {
                ...
                f2[k2] = 1;
                for (let g2 = 1; g2 < l2; g2++) if (i = b[this.rtl ? e - 1 - j - g2 : j + g2], i && i.length >= this.minlength && !f2[i]) {
                  f2[i] = 1;
...
Index.prototype.push_index = function(a, b, c, d, e, f) {
  ...
  if (c && (a = c.query || a, b = c.limit, h = c.offset || 0, e = c.context, f = SUPPORT_SUGGESTION && c.suggest), a && (a = this.encode("" + a), d = a.length, 1 < d)) {
    ...
    for (let e2, h2 = 0, i2 = 0; h2 < d; h2++) if (e2 = a[h2], e2 && e2.length >= this.minlength && !b2[e2]) {
      if (!this.optimize && !f && !this.map[e2]) return g;
      c2[i2++] = e2, b2[e2] = 1;
    }
    a = c2, d = a.length;
  }
  ...
  for (let e2, l; k < d; k++) {
    if (l = a[k], j ? (e2 = this.add_result(g, f, b, h, 2 === d, l, i), (!f || false !== e2 || !g.length) && (i = l)) : e2 = this.add_result(g, f, b, h, 1 === d, l), e2) return e2;
    if (f && k == d - 1) {
      let a2 = g.length;
      if (!a2) {
        if (j) {
      ...
      if (1 === a2) return single_result(g[0], b, h);
    }
  }
  ...
  if (this.optimize || (i = get_array(i, f, g, this.bidirectional)), i) {
    ...
    if (b2) return e ? single_result(h, c, 0) : void (a[a.length] = h);
  }
  return !b && h;
};
...
Index.prototype.contain = function(a) {
  ...
  if (c) {
    if (this.fastupdate) for (let b2, d = 0; d < c.length; d++) b2 = c[d], b2.splice(b2.indexOf(a), 1);
    else remove_index(this.map, a, this.resolution, this.optimize), this.depth && remove_index(this.ctx, a, this.resolution_ctx, this.optimize);
...
function remove_index(a, b, c, d, e) {
  let f = 0;
  if (!is_array(a)) for (let g in a) f = remove_index(a[g], b, c, d, e), f || delete a[g];
  else if (!e) {
