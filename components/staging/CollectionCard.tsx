  ...
  return (
    <div
      ...
      className={`
        ...
        ${isRoot ? 'border-amber-300 bg-amber-50/50' : ''}
      `}
    >
      {/* Drop indicator */}
      {isDragOver && (
        <div className="absolute inset-0 flex items-center justify-center bg-blue-100/80 rounded-xl z-10">
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          {isEditing ? (
            ...
            >
              {collection.name}
              {isRoot && <span className="ml-2 text-[10px] text-amber-600 font-bold">(ROOT)</span>}
            </div>
          )}
      ...
      {manifests.length > 0 && (
        <div className="space-y-1 mt-3 pt-3 border-t border-slate-100">
          {manifests.slice(0, 5).map((manifest) => (
            <div
              key={manifest.id}
              className="group flex items-center gap-2 p-2 bg-slate-50 rounded-lg text-sm"
            >
              <Icon name="menu_book" className="text-emerald-500 text-sm" />
              <span className="flex-1 truncate text-slate-600" title={manifest.name}>
                {manifest.name}
              </span>
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-100 text-red-400 hover:text-red-600 transition-opacity"
      ...
      {manifests.length === 0 && !isDragOver && (
        <div className="mt-3 pt-3 border-t border-dashed border-slate-200 text-center py-4">
          <Icon name="drag_indicator" className="text-2xl text-slate-300 mb-1" />
